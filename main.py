...
def pro_details():
    ...
    if 'logged_in' in session:
        cur = mysql.connection.cursor()
        cur.execute("SELECT DATE_FORMAT(date, '%d/%m/%Y'),GROUP_CONCAT(item_id SEPARATOR ', '),GROUP_CONCAT(quamtity SEPARATOR ', ') from product group by date ")
        userDetails =cur.fetchall()
...
def new_production():
    if request.method == 'POST':
        ...
        quantity_list=quantity.split(',')
        for i in range(len(item_list)):
            cur.execute("INSERT INTO product(date,item_id,quamtity) VALUES (%s,%s, %s)",(date,item_list[i],quantity_list[i]))
        mysql.connection.commit()
        cur.close()
...
def search_pro():
    ...
    if 'logged_in' in session:
        if request.method == 'POST':
            ...
            first_name=userDetails['first_name']
            cur=mysql.connection.cursor()
            cur.execute("SELECT DATE_FORMAT(date, '%%d/%%m/%%Y'),GROUP_CONCAT(item_id SEPARATOR ', '),GROUP_CONCAT(quamtity SEPARATOR ', ') from product where date=%s",[first_name])
            userDetails =cur.fetchall()
...
def upd_pro():
    ...
    if 'logged_in' in session:
        if request.method == 'POST':
            ...
            quantity_list=quantity.split(',')
            for i in range(len(item_list)):
                cur.execute("DELETE from product where date=%s",[date])
            for i in range(len(item_list)):
                    cur.execute("INSERT INTO product(date,item_id,quamtity) VALUES (%s,%s, %s)",(date,item_list[i],quantity_list[i]))
            mysql.connection.commit()
            cur.close()
...
def pur_details():
    ...
    userDetails =cur.fetchall()
    return render_template('purchase.html',userDetails=userDetails,user_image1=pic1)

# @app.routes('/new_pur',methods = ['GET', 'POST'])
# def new_pur():
#     pic1 = os.path.join(app.config['UPLOAD_FOLDER'], 'logo.jpg')
#     if request.method == 'POST':
#         userDetails=request.form
...
app.route('/emp_attendance/<emp_id>',methods = ['GET', 'POST'])
def emp_attendance(emp_id):
    pic1 = os.path.join(app.config['UPLOAD_FOLDER'], 'logo.jpg')
    cur = mysql.connection.cursor()
    cur.execute("SELECT * from attendance where emp_id=%s",[emp_id])
    userDetails=cur.fetchall()
    return render_template('emp_attendance.html',userDetails=userDetails,user_image1=pic1)
